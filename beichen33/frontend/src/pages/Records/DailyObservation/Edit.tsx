...
const DailyObservationEdit: React.FC = () => {
  ...
  // Load existing data into form when record is loaded
  useEffect(() => {
    if (record && !dataLoaded) {
      // Set campus first to trigger teacher loading
      setSelectedCampusId(record.campusId);

      // Convert timeline data
      if (record.timeline && record.timeline.length > 0) {
        const timelineData = record.timeline.map((item: any) => ({
      ...
      form.setFieldsValue({
        ...
        gameActivity: record.gameActivity || '',
        learningActivity: record.learningActivity || '',
        outdoorActivity: record.outdoorActivity || '',
        lifeActivity: record.lifeActivity || '',
        homeCooperation: record.homeCooperation || '',
      });

      setDataLoaded(true);
    }
  }, [record, form, dataLoaded]);

  // Update mutation
  const updateMutation = useMutation({
    mutationFn: (values: any) => dailyObservationApi.update(id!, values),
    onSuccess: () => {
...
export default DailyObservationEdit;
